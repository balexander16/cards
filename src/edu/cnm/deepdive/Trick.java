...
public class Trick {
  ...
  public Trick(){
    ...
    deck = new LinkedList<>();
    for (Suit suit : Suit.values()) { // enhanced for loop. Take collection of Suit.values(). Refer to each item in turn as suit.
      for (Rank rank : Rank.values()) { // same as above for rank.
  ...
  public void swap(){
    ...
    for (int i = 0; i < swapCount; i++){
      blackPile.add(redPile.remove(0)); //add goes to bottom, remove goes from the top!
